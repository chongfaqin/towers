...
def convert_seq2bow_sparse_v2(query,vocab_map,stop_word_dict):
    bow_ids = []
    # query=query.strip()
    # f=set()
    for i, word in enumerate(query.strip().lower()):
        ...
        if word in vocab_map:
            ...
            # bow_ids[vocab_map[conf.unk]] += 1
            bow_ids.append(vocab_map[conf.unk])
            # f.add(vocab_map[conf.unk])

        goods_re=pattern.findall(query.strip().lower())
        for item in goods_re:
            # if(item in f):
            #     continue
            if(item in vocab_map):
                bow_ids.append(vocab_map[item])
            else:
                print(item)
...
def get_data_bow_sparse(file_path):
    ...
    with open(file_path, encoding='utf8') as f:
        for line in f.readlines():
            ...
            query, title, label = spline
            #print(prefix,title)
            query_ids = convert_seq2bow_sparse_v2(query, conf.vocab_map,conf.stop_word_dict)
            #print(prefix_ids)
            title_ids = convert_seq2bow_sparse_v2(title, conf.vocab_map,conf.stop_word_dict)
            #print(title_ids)
...
def get_data_bow_sparse_pred(query,goods_data):
    ...
    :return: [[query, prefix, label]], shape = [n, 3]
    """
    print("vocab_size:",len(conf.vocab_map))
    # print(prefix,title)
    query_ids = convert_seq2bow_sparse_v2(query, conf.vocab_map, conf.stop_word_dict)
    print(query_ids)
    data_arr = []
    ...
    for goods_info in goods_data:
        ...
        if len(goods_info) != 2:
            ...
            continue
        #print(prefix_ids)
        title_ids = convert_seq2bow_sparse_v2(goods_info[1], conf.vocab_map,conf.stop_word_dict)
        #print(title_ids)
        data_arr.append([query_ids, title_ids])
        data_raw.append([goods_info[0],goods_info[1]])
    return data_arr,data_raw
