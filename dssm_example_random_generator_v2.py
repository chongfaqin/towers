...
neg_count=3
vali_random=0.01
file=open("../data/query_title_train_v2.txt","w",encoding="utf-8")
vali_file=open("../data/query_title_vali_v2.txt","w",encoding="utf-8")
for line_arr in line_data:
    ...
    if(key in word_max_min):
        ...
        if(random_val<vali_random):
            vali_file.write("\t".join([key, line_arr[1], str(round(val_nomalise,1))]) + "\n")
        else:
            file.write("\t".join([key, line_arr[1], str(round(val_nomalise, 1))]) + "\n")
    else:
        if(random_val<vali_random):
