...
with tf.name_scope('Loss'):
    ...
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits_v2(labels=doc_label_batch, logits=cos_sim)
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(conf.num_epoch):
        ...
        for batch_id in range(train_epoch_steps):
            # print(batch_id)
            _,loss_v=sess.run([train_step,losses], feed_dict=feed_dict(True, data_train, batch_id, 0.95))
            loss_v=loss_v/query_BS
            epoch_loss += loss_v
        end = time.time()
        ...
        print("Epoch #%d | Train Loss: %-4.3f | PureTrainTime: %-3.3fs" %(epoch, epoch_loss, end - start))
        ...
        print("Epoch #%d | Test  Loss: %-4.3f | Calc_LossTime: %-3.3fs" %(epoch, epoch_loss, end-start))
