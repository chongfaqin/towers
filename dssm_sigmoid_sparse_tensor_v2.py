...
# 读取数据
conf = Config()
data_train = data_input_v2.get_data_bow_sparse(file_train)
data_vali = data_input_v2.get_data_bow_sparse(file_vali)
...
# def _sparse_to_dense(inputs):
#     indixces,values,shape=_sparse_tuple_from(inputs)
#     return tf.sparse_to_dense(indixces, shape, values, default_value=0, validate_indices=True, name=None)

def add_sparse_layer(inputs, in_size, out_size, activation_function=None):
    # inputs=_sparse_to_dense(inputs)
    wlimit = np.sqrt(6.0 / (in_size + out_size))
    Weights = tf.Variable(tf.random_uniform([in_size, out_size], -wlimit, wlimit))
    biases = tf.Variable(tf.random_uniform([out_size], -wlimit, wlimit))
    # biases = tf.Variable(tf.zeros([out_size]))
    # Wx_plus_b = tf.sparse_matmul(inputs, Weights) + biases
    Wx_plus_b = tf.sparse_tensor_dense_matmul(sp_a=inputs, b=Weights)+ biases
    if activation_function is None:
        outputs = Wx_plus_b
    else:
        outputs = activation_function(Wx_plus_b)
...
with tf.name_scope('input'):
    ...
    # query_sparse_batch = tf.placeholder(tf.int64, shape=[None, None], name='query_batch')
    # doc_sparse_batch = tf.placeholder(tf.int64, shape=[None, None], name='doc_batch')
    query_sparse_batch = tf.sparse_placeholder(tf.float32, shape=[None, None], name='query_sparse_batch')
    doc_sparse_batch = tf.sparse_placeholder(tf.float32, shape=[None, None], name='doc_sparse_batch')
...
with tf.name_scope('fc1'):
    query_l1 = add_sparse_layer(query_sparse_batch, conf.nwords, L1_N, activation_function=None)
    doc_l1 = add_sparse_layer(doc_sparse_batch, conf.nwords, L1_N, activation_function=None)

with tf.name_scope('bn1'):
    query_l1 = batch_normalization(query_l1, on_train, "ql1")
    doc_l1 = batch_normalization(doc_l1, on_train, "dl1")
...
with tf.name_scope('bn2'):
    query_l2 = batch_normalization(query_l2, on_train, "ql2")
    doc_l2 = batch_normalization(doc_l2, on_train, "dl2")
...
with tf.name_scope('bn3'):
    query_l3 = batch_normalization(query_l3, on_train, "ql3")
    doc_l3 = batch_normalization(doc_l3, on_train, "dl3")
...
def feed_dict(on_training, data_set, batch_id, drop_prob):
    ...
    return {query_sparse_batch: tf.SparseTensorValue(q_index,q_values,q_shape),
            doc_sparse_batch: tf.SparseTensorValue(d_index,d_values,d_shape),
            doc_label_batch: np.array(label),on_train: on_training, keep_prob: drop_prob}
...
with tf.Session(config=config) as sess:
    ...
    builder = tf.saved_model.builder.SavedModelBuilder("dssm_pb_model_v2/")
