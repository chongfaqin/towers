...
# 读取数据
conf = Config()
data_train,data_raw = data_input.get_data_bow_sparse_pred("儿童口罩",goods_data)
...
with tf.Session(config=config) as sess:
    ...
    for batch_id in range(train_epoch_steps):
        title_semantic_vector,logits_val = sess.run([doc_pred,logits], feed_dict=feed_dict(data_train,batch_id))
